...
## Description
Context priming skill for the primer subagent. Spawned at the start of every agent session, this skill classifies the session mode (CONTINUE, BUILD, or PLAN), loads relevant context from the project, and distills it into a tight context block for the main agent. The primer subagent's context window is disposable — it can load broadly and reason about relevance without polluting the main agent's context.

## Triggers
- Every new agent session (always — this skill runs before any other)
- Main agent spawns the primer subagent and passes the developer's opening message

## Steps
1. Check for unresolved conflicts: read `.cx/conflicts.json`. If it exists and contains entries, spawn a conflict-resolver subagent (using cx-conflict-resolve skill) BEFORE proceeding. Wait for resolution to complete.
...
7. Return this context block to the main agent. The main agent uses it as its starting context.

## Rules
- Never make changes to any files — the primer is read-only
- Never interact with the developer directly — the primer only communicates with the main agent
